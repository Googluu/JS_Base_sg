...
function Header() {
    ...
    if (isAuthenticated) {
        menu = <Link to="/profile" className="block mt-4 lg:inline-block lg:mt-0 text-gray-800 hover:text-dark-gray mr-4 sm:w-auto">
                    <span role="img" aria-label="Profile">⚙️</span> Profile
               </Link>
    } else {
        menu =  <Link to="/signup" className="block mt-4 lg:inline-block lg:mt-0 text-gray-800 hover:text-dark-gray mr-4 sm:w-auto">
                    <span role="img" aria-label="Sign Up Gear">✍️</span> Sign Up
                </Link>
    }
